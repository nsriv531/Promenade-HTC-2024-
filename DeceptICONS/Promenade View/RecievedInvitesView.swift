    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 12) {
                Text("Recieved invites")
                    .exposureFont()
                    .frame(maxWidth: .infinity, alignment: .leading)

                ForEach(firebase.receivedInvites.sorted(by: { $0.toUser.firstName < $1.toUser.firstName })) { invite in
                    InviteView(invite: invite)
                }
            }
            .padding()
        }
        .refreshable(action: firebase.fetchReceivedInvites)
